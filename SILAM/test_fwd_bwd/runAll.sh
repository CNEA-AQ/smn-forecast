...
start_date="2009/03/15 18:00" #"2021/12/10 10:00" # 2017/01/01 07:00" #No poner los segundos!
computed_period="10 hours"

bbox=(-65. -50. -55. -40.)  #(-10. 60.0 0.0 70.0) #(lonmin latmin lonmax latmax)
								
dynamic_meteo_file="NETCDF:WRF_OUTPUT met\/wrfout_d01_2009-03-15_18" #"GRIB ..\/EUR2016_ec100_137ARG\/2021\/ecE100_137_ARG%ay4%am2%ad2%ah2+%f3.ml" #
 static_meteo_file="NETCDF:WRF_OUTPUT met\/wrfout_d01_2009-03-15_18" #"GRIB ..\/EUR2016_ec100_137ARG\/ec100_137_ARGstatic.ml"                      #
#-------------------------

end_date=$(date -d "${computed_period} $start_date" +"%Y/%m/%d %H:00")
...
then
	...
	sed -i "s/ case_name = .*/ case_name = ${testname}_/" ${ctlfile}
        ...
        sed -i "s/ dynamic_meteo_file = .*/ dynamic_meteo_file = ${dynamic_meteo_file}/" ${ctlfile} 
        sed -i "s/ static_meteo_file = .*/ static_meteo_file = ${static_meteo_file}/" ${ctlfile}

	#Remove if repeated:
	awk 'BEGIN{ i=1 }{ if(/dynamic_meteo_file/){ if(i==1){print $0}else{};i++}else{print $0}}' ${ctlfile} > tmp
	#awk 'BEGIN{ i=1 }{ if(/static_meteo_file/){ if(i==1){print $0}else{};i++}else{print $0}}' tmp > ${ctlfile}
	cat tmp > ${ctlfile}
	rm tmp

	if [ ${dynamic_meteo_file// .*/} == "GRIB" ]
	then
		sed -i "/ dynamic_meteo_file = .*/a\ dynamic_meteo_file = ${dynamic_meteo_file//\.ml/\.sfc}" ${ctlfile}
	fi
	#if [ ${static_meteo_file// .*/} == "GRIB" ]
        #then
	#	sed -i "/ static_meteo_file = .*/a\ static_meteo_file = ${static_meteo_file//\.ml/\.sfc}" ${ctlfile}
        #fi
...
POINT_SOURCE_5 # First point-source starts
   ...
   par_str_point = ${start_date//[-:\/]/ } 0.0  1.    500. 1000.   5.0   450.  PASSIVE_COCKTAIL 25.  
   par_str_point = ${start_date1//[-:\/]/ } 0.0  1.    500. 1000.   5.0   450.  PASSIVE_COCKTAIL 25.
...
EOF
	...
	sed -i "s/ start_time = .*/ start_time = ${start_date//[:\/]/ } 0.0/" ${ctlfile}
	sed -i "s/ direction_in_time = .*/ direction_in_time = FORWARD/" ${ctlfile}
	sed -i "s/ emission_source = .*/ emission_source = EULERIAN ${srcfile_fwd}/" ${ctlfile}
...
POINT_SOURCE_5 # First point-source starts
   ...
   par_str_point = ${end_date1//[-:\/]/ } 0.0  1.    500. 1000.   5.0   450.  PASSIVE_COCKTAIL 25.  
    par_str_point = ${end_date//[-:\/]/ } 0.0  1.    500. 1000.   5.0   450.  PASSIVE_COCKTAIL 25.
...
EOF
        ...
        #Changing control file parameters:
        sed -i "s/ start_time = .*/ start_time = ${end_date//[:\/-]/ } 0.0/" ${ctlfile}
        sed -i "s/ direction_in_time =.*/ direction_in_time = BACKWARD/" ${ctlfile}
	sed -i "s/ emission_source = .*/ emission_source = EULERIAN ${srcfile_bwd}/" ${ctlfile}
...
#dx=0.5   					#dx=0.5 (Norway)                        #dx=0.5 (Arg. ECMWF)     
#| dt      | Error measure|                     #| dt     |Error measure|               #| dt     |Error measure|
#|---------|--------------|                     #|--------|-------------|               #|--------|-------------|
#|  03min  |   0.63104    |                     #| 03min  |  0.367032   |		#| 3      |  0.707862   | 
#|  05min  |   0.568971   | 	                #| 05min  |  0.333031   |               #| 5      |  0.671696   | 
#|  10min  |   0.515291   | <-- optimal         #| 10min  |  0.296046   |               #| 10     |  0.608899   | 
#|  15min  |   0.51597    |                     #| 15min  |  0.263579   | <-- optimal   #| 15     |  0.595284   | 
#|  25min  |   0.768848   |                     #| 25min  |  0.532827   |               #| 25     |  0.6539     | 
#|  30min  |   0.711312   |                     #| 30min  |  0.359187   |               #| 30     |  0.544847   | <--opt.
#|  60min  |   0.827925   |                     #| 60min  |  0.748884   |               #| 60     |  0.628879   | 


#dt=15min					#dt=15min                               #dt=15min
#| dx=dy   | Error measure|			#| dx=dy  | Error measure |             #| dx=dy  | Error measure |
#|---------|--------------|                     #|--------|---------------|             #|--------|---------------|
#|  0.05   |  0.693682	  |                     #| 0.05   |    0.474032   |  		#|  0.05  |  0.567996     |
#|  0.1    |  0.628369	  |                     #| 0.1    |    0.515637   |             #|  0.1   |  0.598941     |
#|  0.25   |  0.607903	  |                     #| 0.25   |    0.390781   |             #|  0.25  |  0.729354     |
#|  0.5    |  0.51597	  |                     #| 0.5    |    0.263579   | <-- optimal #|  0.5   |  0.560214     |
#|  0.75   |  0.485843	  | <-- optimal         #| 0.75   |    0.282655   |             #|  0.75  |  0.595418     |
#|  1.0    |  0.593787	  |                     #| 1.0    |    0.329181   |             #|  1.0   |  0.636502     |
