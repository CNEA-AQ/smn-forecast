...
CONTAINS
  ...
  subroutine fill_bio_voc_src_from_namelist(nlSetup, bvoc_src, src_data_dir)
    ...
    select case(fu_str_u_case(fu_content(nlSetup,'bvoc_emission_method')))
      ...
      case ('MEGANv3.2')                                           !megan
        bvoc_src%emisMethod = emis_megan_v32                       !megan
        bvoc_src%nspecies   = 29 !N_CB05_SPC                       !megan
        !SILAM CBM5 LIST:       ---------------------------------------
        !'BENZENE','SESQ   ','O1D    ','TOL    ','ETHA   ',           | 
        ...
        !'PAR5   ','XO2N   ','NO     ','OH     '                      |
        !--------------------------------------------------------------
        call set_species(speciesTmp, fu_get_material_ptr('C5H8   '), in_gas_phase); bvoc_src%species( 1) = speciesTmp !megan ISOP  1   
        call set_species(speciesTmp, fu_get_material_ptr('C5H8_2 '), in_gas_phase); bvoc_src%species( 2) = speciesTmp !megan TERP  2  
