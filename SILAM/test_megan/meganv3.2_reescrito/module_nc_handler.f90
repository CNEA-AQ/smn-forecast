module nc_handler_mod
        ...
        use proj_mod
  implicit none
  !interface get_2d_var_from_nc
  !     module procedure get2DvarFromNetCDFint
  !     module procedure get2DvarFromNetCDF
  ! end interface
  !interface get_3d_var_from_nc
  !     module procedure get3DvarFromNetCDFint
  !     module procedure get3DvarFromNetCDF
  !end interface
...
contains
 ...
 subroutine createNetCDF(outFile,p,g,var_list,var_unit,var_desc,var_dtyp)
    ...
    character(len=3)  :: var_list(:)
    character(len=5) :: var_unit(:),var_dtyp(:)
    ...
    call check(nf90_create(outFile, NF90_CLOBBER, ncid))
        ...
        do k=1, nvars
          print*," creando var:",trim(var_list(k)),trim(var_dtyp(k)),trim(var_unit(k))
 ...
 function get_2d_var_from_nc(ncfile, varname, nx, ny)  result(GRIDVAR)
    implicit none
 ...
 function get_2d_var_from_nc_int(ncfile, varname, nx, ny)  result(GRIDVAR)
    implicit none
    character(*), intent(in) :: ncfile
    character(*) , intent(in) :: varname
    integer, intent(in)       :: nx,ny
...
function get_3d_var_from_nc_int(ncfile, varname,nx,ny,nt)  result(GRIDVAR)
   implicit none
   character(*), intent(in)  :: ncfile
   character(*) , intent(in) :: varname
...
function get_3d_var_from_nc(ncfile, varname,nx,ny,nt)  result(GRIDVAR)
   implicit none
   character(*), intent(in)  :: ncfile
   character(*) , intent(in) :: varname
   integer, intent(in)       :: nx,ny,nt
