...
! Netcdf file
      CHARACTER*16  :: CANTYP = 'CANTYPFILE'     ! canopy type file logical name
      CHARACTER*16  :: LAIFILE = 'LAIFILE'     ! LAI file logical name
! Met files
...
! I/O API file parameters
      ...
      INTEGER :: NLAI         ! Number of LAI values (46 for 8-day; 37
                              ! for 10-day; 12 for monthly LAI)
...
! LAI file
      MESG = 'Number of LAI values'
      NLAI = ENVINT( 'NLAI', MESG, NLAI, IOS )
      IF ( NLAI .NE. 46 .AND. NLAI .NE. 37 .AND. NLAI .NE. 12 ) THEN
           STOP 'Bad value for NLAI. Only 46, 37 or 12 is allowed'
      ENDIF
      IF ( .NOT. OPEN3( LAIFILE, FSREAD3, PROGNAME ) ) THEN
         CALL NAMEVAL (LAIFILE, MESG)  ! get input file name and path
         MESG = 'Could not open file '//TRIM(MESG)
         CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
      ENDIF
      ! Check grid
      IF ( .NOT. FILCHK3 ( LAIFILE,
     &              GRDDED3, NCOLS3D, NROWS3D, 1, NTHIK3D))  THEN
         MESG = 'LAIFILE has differenet grid definition'
         CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
      ENDIF
      IF ( .NOT. DESC3( LAIFILE ) ) THEN
         CALL NAMEVAL (LAIFILE, MESG)  ! get input file name and path
         MESG = 'Could not get description of '//TRIM(MESG)
         CALL M3EXIT( PROGNAME, 0, 0, MESG, 2 )
      ENDIF
...
!...  Read LAIFILE
      ...
      IF ( .NOT. READ3(LAIFILE,'LAT',1,0,0,LAT)) THEN
         MESG = 'Error reading LAT'
         CALL M3EXIT(PROGNAME,IDATE,ITIME,MESG,2)
      ENDIF

      IF ( .NOT. READ3(LAIFILE,'LONG',1,0,0,LONG)) THEN
...
!...  Initialize hourly variables
        ...
        ! Find LAIc from date
        CALL FINDLAI(IDATE,MXLAI,NLAI,LAIp_I,LAIc_I)
        WRITE(MESG,1020) 'Found LAI current period for YYYYJJJ : ',
     &                    IDATE,LAIc_I
        CALL M3MESG( MESG )
        WRITE(MESG,'(I0.2)') LAIc_I
        IF ( .NOT. READ3(LAIFILE,'LAI'//TRIM(MESG),ALLAYS3,0,0,LAIc)) THEN
...
! ... Exit and close file
      ...
      DEALLOCATE ( LAIc    )   ! current time step LAI
