...
set IN_PTpath = $EMISpath/inln_point              #> point source emissions input directory
set IN_LTpath = $EMISpath/lightning               #> lightning NOx input directory
...
set NSTEPS     = 120000           #> time duration (HHMMSS) for this run
...
while ($TODAYJ <= $STOP_DAY )  #>Compare dates in terms of YYYYJJJ
  ...
  if ($NEW_START == true || $NEW_START == TRUE ) then
     setenv ICFILE ICON_v532_${APPL}_profile_20221019
  ...
  #> Boundary conditions
  set BCFILE = BCON_v532_${APPL}_profile_20221019
  ...
  setenv GR_EMIS_001 ${EMISpath}/finn/${EMISfile}
  setenv GR_EMIS_LAB_001 GRIDDED_EMIS
  setenv GR_EM_SYM_DATE_001 F # To change default behaviour please see Users Guide for EMIS_SYM_DATE
  ...
  if ( $CTM_BIOGEMIS_MG == 'Y' ) then                                       
     ...
     #Acá defino la ubic. de los archivos generados con prepmegan4cmaq:
     setenv MEGAN_CTS ${EMISpath}/megan/CT3.nc                    
     setenv MEGAN_EFS ${EMISpath}/megan/EF.nc
     setenv MEGAN_LDF ${EMISpath}/megan/LDF.nc                    
     setenv MEGAN_LAI ${EMISpath}/megan/LAI3.nc

     if ($BDSNP_MEGAN == 'Y') then                                      
        setenv BDSNPINP    $OUTDIR/CCTM_BDSNPOUT_${RUNID}_${YESTERDAY}.n
        setenv BDSNP_FFILE  ${EMISpath}/megan/FERT_rami.ncf          
        setenv BDSNP_NFILE  ${EMISpath}/megan/NDEP_rami.ncf          
        setenv BDSNP_LFILE  ${EMISpath}/megan/LANDTYPE_rami.ncf      
        setenv BDSNP_AFILE  ${EMISpath}/megan/ARID_rami.ncf          
        setenv BDSNP_NAFILE ${EMISpath}/megan/NONARID_rami.ncf      
     endif                 
  endif
...
( /usr/bin/time -p mpiexec -np $NPROCS $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt 
